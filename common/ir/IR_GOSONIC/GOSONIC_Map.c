...
#include "GOSONIC_Map.h"

typedef	struct _stGOSONIC_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stGOSONIC_DTVKeymap;

int stGOSONIC_Keymap[0x100] = {
...
/*0x1c, 0x1d ,0x1e, 0x1f*/	CMD_DVB_T_C_MODE, CMD_AUDIO, CMD_NOP, CMD_SLEEP,
...
/*0x40, 0x41 ,0x42, 0x43*/	CMD_APP, CMD_CHANNEL_DEC, CMD_DISPLAY, CMD_PVR_RECORD,
...
stGOSONIC_DTVKeymap		TVKeyMapTab_GOSONIC[]=
{
...
stGOSONIC_DTVKeymap		DTVKeyMapTab_GOSONIC[]=
{	
	{CMD_PLAY_PAUSE,	                    CMD_TELETEXT},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stGOSONIC_DTVKeymap		AVKeyMapTab_GOSONIC[]=
{

	{CMD_NOP,					CMD_NOP},
};
stGOSONIC_DTVKeymap		PCKeyMapTab_GOSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stGOSONIC_DTVKeymap		HDMIKeyMapTab_GOSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stGOSONIC_DTVKeymap		MEDIAKeyMapTab_GOSONIC[]=
{	
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_NOP,					CMD_NOP},
};
stGOSONIC_DTVKeymap		DVDKeyMapTab_GOSONIC[]=
{	// IR_CMD					DVD_CODE
	
	{CMD_NOP,					CMD_NOP},
};
stGOSONIC_DTVKeymap		DefaultKeyMapTab_GOSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stGOSONIC_DTVKeymap		TTKeyMapTab_GOSONIC[]=
{
	{CMD_PLAY_PAUSE,	            CMD_TELETEXT},
	{CMD_STOP,	             		CMD_WIDE},
...
static stGOSONIC_DTVKeymap DVDKeyMapTab[]=
{// IR_CMD				DVD_CODE
	{CMD_LEFT,   		0x02},	
	{CMD_RIGHT,  		0x40},
	{CMD_UP,  		    0x03},	
...
/*************************************************************************************************************************************/
extern stGOSONIC_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stGOSONIC_DTVKeymap *pDVD_key = ( stGOSONIC_DTVKeymap *)DVDKeyMapTab;
...
int remote_GOSONIC_nec(int code)
{
	stGOSONIC_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	...
	if((code & 0x0000ffff) != GOSONIC_CUSTOMER)
	            return -1;
	...
	switch (source)
	{
		case SOURCE_IDTV1:
			...
			if(HK_GetTT_Enable())
				pKeymap=TTKeyMapTab_GOSONIC;
			else
			//modify by zjl@2014.09.01--start	
			{
				if(TvApi_GetIDtvSubSource() >= 1)
					pKeymap=DTVKeyMapTab_GOSONIC;
				else
					pKeymap=TVKeyMapTab_GOSONIC;
			}
		...
		case SOURCE_PLAYBACK://add by zjl@2014.06.11
			pKeymap=MEDIAKeyMapTab_GOSONIC;
			break;
...
#endif
		default:
			pKeymap=DefaultKeyMapTab_GOSONIC;
			break;
