...
typedef	struct _stCHFAC_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
} stCHFAC_DTVKeymap;

int stCHFAC_Keymap[0x100] =
{
...
int stCHFAC_NOT_M_Keymap[0x100] =
{
	/*0x00, 0x01 ,0x02, 0x03*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
	/*0x04, 0x05 ,0x06, 0x07*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
	/*0x08, 0x09 ,0x0a, 0x0b*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_PANEL_INDEX,
	/*0x0c, 0x0d ,0x0e, 0x0f*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,

	/*0x10, 0x11 ,0x12, 0x13*/	CMD_M_MODE, CMD_NOP, CMD_POWER, CMD_NOP,
	/*0x14, 0x15 ,0x16, 0x17*/	CMD_MENU, CMD_NOP, CMD_NOP, CMD_NOP,
	...
	/*0x40, 0x41 ,0x42, 0x43*/	CMD_SOURCE, CMD_NOP, CMD_NOP, CMD_NOP,
...
stCHFAC_DTVKeymap		TVKeyMapTab_CHFAC[] =
{
...
stCHFAC_DTVKeymap		DTVKeyMapTab_CHFAC[] =
{
	// {CMD_STOP,					CMD_FREEZE},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stCHFAC_DTVKeymap		AVKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
...
stCHFAC_DTVKeymap		PCKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
	// {CMD_CHANNEL_INC,			CMD_SOURCE_ATV},
	// {CMD_CHANNEL_DEC,			CMD_SOURCE_ATV},
...
stCHFAC_DTVKeymap		HDMIKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
...
stCHFAC_DTVKeymap		MEDIAKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
...
stCHFAC_DTVKeymap		DVDKeyMapTab_CHFAC[] =
{
	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stCHFAC_DTVKeymap		DefaultKeyMapTab_CHFAC[] =
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};

stCHFAC_DTVKeymap		TTKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
...
stCHFAC_DTVKeymap		RECKeyMapTab_CHFAC[] =
{
	// {CMD_DVB_T_C_MODE,          CMD_NOP},
	// {CMD_TELETEXT,				CMD_PLAY_PAUSE},
	{CMD_OPTION_YELLOW,			CMD_PREV},
...
static stCHFAC_DTVKeymap DVDKeyMapTab[] =
{
...
/*************************************************************************************************************************************/
extern stCHFAC_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stCHFAC_DTVKeymap *pDVD_key = ( stCHFAC_DTVKeymap *)DVDKeyMapTab;
...
int remote_CHFAC_nec(int code)
{
	stCHFAC_DTVKeymap* pKeymap = NULL;
	int   cDscIrKey = 0;
	int   cDscIrSWKey = 0;
	int i = 0;
	...
	if ((code & 0x0000ffff) != CHFAC_CUSTOMER)
		return -1;
	...
	if (M_mode == false)
	{
		while (i <= 0xFF) //modify by zjl@2014.10.22
		{
			...
			{
				cDscIrKey = stCHFAC_NOT_M_Keymap[i];
				break;
			}
			i++;
		}
			...
			//add by lq@2014.12.16--start
		{
			if (TvApi_GetIDtvSubSource() >= 1)
			{
				if ( TvApi_IsRecording() == true )
					pKeymap = RECKeyMapTab_CHFAC;
				else
					pKeymap = DTVKeyMapTab_CHFAC;
			}
			else
			{
				if ( TvApi_IsRecording() == true )
	...
	case SOURCE_HDMI3:
		pKeymap = HDMIKeyMapTab_CHFAC;
		break;

	case SOURCE_BROWSER:
	case SOURCE_OSD:
	case SOURCE_PLAYBACK://add by zjl@2014.06.11
		pKeymap = MEDIAKeyMapTab_CHFAC;
		break;
...
#endif
	default:
		pKeymap = DefaultKeyMapTab_CHFAC;
		break;
