...
#include "HDRD19_Map.h"

typedef	struct _stHDRD19_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stHDRD19_DTVKeymap;

int stHDRD19_Keymap[0x100] = {
/*0x00, 0x01 ,0x02, 0x03*/	CMD_ONE, CMD_NOP, CMD_VOL_UP, CMD_CHANNEL_INC,
/*0x04, 0x05 ,0x06, 0x07*/	CMD_EPG, CMD_NOP, CMD_MENU, CMD_FAVCH_DOWN,
/*0x08, 0x09 ,0x0a, 0x0b*/	CMD_CAPTION, CMD_VOL_DOWN, CMD_EXIT, CMD_PICTURE_MODE,
/*0x0c, 0x0d ,0x0e, 0x0f*/	CMD_NOP, CMD_FAVCH_UP, CMD_NOP, CMD_DISPLAY_RATIO,
...
/*0x48, 0x49 ,0x4a, 0x4b*/	CMD_FAVCH, CMD_LEFT, CMD_ENTER, CMD_RIGHT,
...
/*0x58, 0x59 ,0x5a, 0x5b*/	CMD_NOP, CMD_TELETEXT_SUBTITLE, CMD_NOP, CMD_STOP,
...
stHDRD19_DTVKeymap		TVKeyMapTab_HDRD19[]=
{
...
stHDRD19_DTVKeymap		DTVKeyMapTab_HDRD19[]=
{	
	{CMD_PLAY_PAUSE,	                    CMD_TELETEXT},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stHDRD19_DTVKeymap		AVKeyMapTab_HDRD19[]=
{

	{CMD_NOP,					CMD_NOP},
};
stHDRD19_DTVKeymap		PCKeyMapTab_HDRD19[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stHDRD19_DTVKeymap		HDMIKeyMapTab_HDRD19[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stHDRD19_DTVKeymap		MEDIAKeyMapTab_HDRD19[]=
{	
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_NOP,					CMD_NOP},
};
stHDRD19_DTVKeymap		DVDKeyMapTab_HDRD19[]=
{	// IR_CMD					DVD_CODE
	
	{CMD_NOP,					CMD_NOP},
};
stHDRD19_DTVKeymap		DefaultKeyMapTab_HDRD19[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stHDRD19_DTVKeymap		TTKeyMapTab_HDRD19[]=
{
	{CMD_PLAY_PAUSE,	            CMD_TELETEXT},
	{CMD_STOP,	             		CMD_WIDE},
...
static stHDRD19_DTVKeymap DVDKeyMapTab[]=
{// IR_CMD				DVD_CODE
	{CMD_LEFT,   		0x02},	
	{CMD_RIGHT,  		0x40},
	{CMD_UP,  		    0x03},	
...
/*************************************************************************************************************************************/
extern stHDRD19_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stHDRD19_DTVKeymap *pDVD_key = ( stHDRD19_DTVKeymap *)DVDKeyMapTab;
...
int remote_HDRD19_nec(int code)
    {
        stHDRD19_DTVKeymap* pKeymap=NULL;
        int   cDscIrKey=0;
        int   cDscIrSWKey=0;
        int i =0;
        ...
        if((code & 0x0000ffff) != HDRD19_CUSTOMER)
            return -1;
        ...
        switch (source)
        {
                ...
                //add by lq@2014.12.16--start
                {
                    if(TvApi_GetIDtvSubSource() >= 1)
                    {
                        if( TvApi_IsRecording() == true )
                            pKeymap=RECKeyMapTab_HDRD19;
                        else
                            pKeymap=DTVKeyMapTab_HDRD19;
                    }
                    else
                    {
                        if( TvApi_IsRecording() == true )
            ...
            case SOURCE_HDMI3:
                pKeymap=HDMIKeyMapTab_HDRD19;
                break;
            
            case SOURCE_BROWSER:
            case SOURCE_OSD:
            case SOURCE_PLAYBACK://add by zjl@2014.06.11
                pKeymap=MEDIAKeyMapTab_HDRD19;
                break;
...
#endif
            default:
                pKeymap=DefaultKeyMapTab_HDRD19;
                break;
