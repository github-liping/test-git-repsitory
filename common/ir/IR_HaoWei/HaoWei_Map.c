...
#include "HaoWei_Map.h"

typedef	struct _stHaoWei_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stHaoWei_DTVKeymap;

int stHaoWei_Keymap[0x100] = {
...
stHaoWei_DTVKeymap		TVKeyMapTab_HaoWei[]=
{
	{CMD_TELETEXT_SUBTITLE,		CMD_STITLE},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--start
stHaoWei_DTVKeymap		DTVKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stHaoWei_DTVKeymap		AVKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stHaoWei_DTVKeymap		PCKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stHaoWei_DTVKeymap		HDMIKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stHaoWei_DTVKeymap		MEDIAKeyMapTab_HaoWei[]=
{
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_TT_TXCL,				CMD_NOP},
...
stHaoWei_DTVKeymap		DVDKeyMapTab_HaoWei[]=
{	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stHaoWei_DTVKeymap		DefaultKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stHaoWei_DTVKeymap		TTKeyMapTab_HaoWei[]=
{
...
stHaoWei_DTVKeymap		RECKeyMapTab_HaoWei[]=
{
	{CMD_NOP,					CMD_NOP},
};
...
static stHaoWei_DTVKeymap DVDKeyMapTab[]=
{
...
/*************************************************************************************************************************************/
extern stHaoWei_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stHaoWei_DTVKeymap *pDVD_key = ( stHaoWei_DTVKeymap *)DVDKeyMapTab;
...
int remote_HaoWei_nec(int code)
{
	stHaoWei_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	...
	if((code & 0x0000ffff) != HaoWei_CUSTOMER)
	    return -1;
	...
	switch (source)
	{
			...
			//add by lq@2014.12.16--start
			{
				if(TvApi_GetIDtvSubSource() >= 1)
				{
					...
					if( TvApi_IsRecording() == true )
						pKeymap=RECKeyMapTab_HaoWei;
					else
					#endif
						pKeymap=DTVKeyMapTab_HaoWei;
				}
				else
				{
					#ifndef BUILD_TV036_2_ISDB	
		...
		case SOURCE_HDMI3:
			pKeymap=HDMIKeyMapTab_HaoWei;
			break;
		
		case SOURCE_BROWSER:
		case SOURCE_OSD:
		case SOURCE_PLAYBACK://add by zjl@2014.06.11
			pKeymap=MEDIAKeyMapTab_HaoWei;
			break;
...
#endif
		default:
			pKeymap=DefaultKeyMapTab_HaoWei;
			break;
...
int stHaoWei_Keymap[0x100] = {
	#if 0
/*0x00, 0x01 ,0x02, 0x03*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_STITLE,
/*0x04, 0x05 ,0x06, 0x07*/	CMD_NOP, CMD_NOP, CMD_UP, CMD_SLEEP,
/*0x08, 0x09 ,0x0a, 0x0b*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x0c, 0x0d ,0x0e, 0x0f*/	CMD_NINE, CMD_VOL_UP, CMD_NOP, CMD_ZERO,

/*0x10, 0x11 ,0x12, 0x13*/	CMD_THREE, CMD_VOL_DOWN, CMD_NOP, CMD_NOP,
/*0x14, 0x15 ,0x16, 0x17*/	CMD_SIX, CMD_RETURN, CMD_DOWN, CMD_POINT,
/*0x18, 0x19 ,0x1a, 0x1b*/	CMD_NOP, CMD_MUTE, CMD_ENTER, CMD_RIGHT,
...
/*0x40, 0x41 ,0x42, 0x43*/	CMD_DISPLAY_RATIO, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x44, 0x45 ,0x46, 0x47*/	CMD_NOP, CMD_SOURCE, CMD_NOP, CMD_NOP,
/*0x48, 0x49 ,0x4a, 0x4b*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x4c, 0x4d ,0x4e, 0x4f*/	CMD_EIGHT, CMD_CHANNEL_INC, CMD_SEVEN, CMD_NOP,

/*0x50, 0x51 ,0x52, 0x53*/	CMD_TWO, CMD_CHANNEL_DEC, CMD_ONE, CMD_NOP,
/*0x54, 0x55 ,0x56, 0x57*/	CMD_FIVE, CMD_MENU, CMD_FOUR, CMD_NOP,
/*0x58, 0x59 ,0x5a, 0x5b*/	CMD_NOP, CMD_POWER, CMD_LEFT, CMD_NOP,
/*0x5c, 0x5d ,0x5e, 0x5f*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,

/*0x60, 0x61 ,0x62, 0x63*/	CMD_FAVCH, CMD_DISPLAY, CMD_JUMP, CMD_NOP,
/*0x64, 0x65 ,0x66, 0x67*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x68, 0x69 ,0x6a, 0x6b*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x6c, 0x6d ,0x6e, 0x6f*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
...
/*0x08, 0x09 ,0x0a, 0x0b*/	CMD_NOP, CMD_NOP, CMD_VOL_UP, CMD_NOP,
/*0x0c, 0x0d ,0x0e, 0x0f*/	CMD_DISPLAY, CMD_NOP, CMD_POINT, CMD_NOP,

/*0x10, 0x11 ,0x12, 0x13*/	CMD_STITLE, CMD_STOP, CMD_SEVEN, CMD_ZERO,
/*0x14, 0x15 ,0x16, 0x17*/	CMD_OPTION_BLUE, CMD_NOP, CMD_FOUR, CMD_EIGHT,
/*0x18, 0x19 ,0x1a, 0x1b*/	CMD_RETURN, CMD_AUDIO, CMD_ONE, CMD_FIVE,
...
/*0x40, 0x41 ,0x42, 0x43*/	CMD_RIGHT, CMD_NOP, CMD_DISPLAY_RATIO, CMD_FRWD,
/*0x44, 0x45 ,0x46, 0x47*/	CMD_CHANNEL_DEC, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x48, 0x49 ,0x4a, 0x4b*/	CMD_CHANNEL_INC, CMD_PREV, CMD_NEXT, CMD_NOP,
/*0x4c, 0x4d ,0x4e, 0x4f*/	CMD_JUMP, CMD_EPG, CMD_PLAY, CMD_FFWD,

/*0x50, 0x51 ,0x52, 0x53*/	CMD_NINE, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x54, 0x55 ,0x56, 0x57*/	CMD_SIX, CMD_OPTION_RED, CMD_OPTION_GREEN, CMD_OPTION_YELLOW,
/*0x58, 0x59 ,0x5a, 0x5b*/	CMD_THREE, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x5c, 0x5d ,0x5e, 0x5f*/	CMD_MUTE, CMD_SOURCE_DTV, CMD_DOWN, CMD_ENTER,

/*0x60, 0x61 ,0x62, 0x63*/	CMD_FAVCH, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x64, 0x65 ,0x66, 0x67*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x68, 0x69 ,0x6a, 0x6b*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
/*0x6c, 0x6d ,0x6e, 0x6f*/	CMD_NOP, CMD_NOP, CMD_NOP, CMD_NOP,
