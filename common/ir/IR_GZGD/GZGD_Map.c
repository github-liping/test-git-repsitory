...
typedef	struct _stGZGD_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stGZGD_DTVKeymap;

int stGZGD_Keymap[0x100] = {
...
/*0x1c, 0x1d ,0x1e, 0x1f*/	CMD_SOURCE_VGA, CMD_SOURCE_AV, CMD_NOP, CMD_HDMI,
...
stGZGD_DTVKeymap		TVKeyMapTab_GZGD[]=
{
	{CMD_TELETEXT_SUBTITLE,		CMD_STITLE},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--start
stGZGD_DTVKeymap		DTVKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stGZGD_DTVKeymap		AVKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stGZGD_DTVKeymap		PCKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stGZGD_DTVKeymap		HDMIKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stGZGD_DTVKeymap		MEDIAKeyMapTab_GZGD[]=
{
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_TT_TXCL,				CMD_NOP},
...
stGZGD_DTVKeymap		DVDKeyMapTab_GZGD[]=
{	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stGZGD_DTVKeymap		DefaultKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stGZGD_DTVKeymap		TTKeyMapTab_GZGD[]=
{
...
stGZGD_DTVKeymap		RECKeyMapTab_GZGD[]=
{
	{CMD_NOP,					CMD_NOP},
};
...
static stGZGD_DTVKeymap DVDKeyMapTab[]=
{
...
int remote_GZGD_nec(int code)
{
	stGZGD_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	...
	if((code & 0x0000ffff) != GZGD_CUSTOMER)
	    return -1;
	...
	switch (source)
	{
		case SOURCE_IDTV1:
			...
			if(HK_GetTT_Enable())
				pKeymap=TTKeyMapTab_GZGD;
			else	
			//add by lq@2014.12.16--start
			{
				if(TvApi_GetIDtvSubSource() >= 1)
					pKeymap=DTVKeyMapTab_GZGD;
				else
					pKeymap=TVKeyMapTab_GZGD;
			}
			//add by lq@2014.12.16--end
#else
		...
		case SOURCE_HDMI3:
			pKeymap=HDMIKeyMapTab_GZGD;
			break;
		
		case SOURCE_BROWSER:
		case SOURCE_OSD:
		case SOURCE_PLAYBACK://add by zjl@2014.06.11
			pKeymap=MEDIAKeyMapTab_GZGD;
			break;
