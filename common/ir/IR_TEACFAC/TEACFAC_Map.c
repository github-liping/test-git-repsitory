...
#include "TEACFAC_Map.h"

typedef	struct _stTEACFAC_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stTEACFAC_DTVKeymap;

int stTEACFAC_Keymap[0x100] = {
...
/*0x84, 0x85 ,0x86, 0x87*/	CMD_GOTO, CMD_NOP, CMD_NOP, CMD_AB_REPEAT,
/*0x88, 0x89 ,0x8a, 0x8b*/	CMD_ONE, CMD_TWO, CMD_THREE, CMD_EPG,
/*0x8c, 0x8d ,0x8e, 0x8f*/	CMD_FOUR, CMD_FIVE, CMD_SIX, CMD_CHANNEL_DEC,

/*0x90, 0x91 ,0x92, 0x93*/	CMD_SEVEN, CMD_EIGHT, CMD_NINE, CMD_EXIT,
/*0x94, 0x95 ,0x96, 0x97*/	CMD_FAVCH, CMD_ZERO, CMD_JUMP, CMD_MENU,
/*0x98, 0x99 ,0x9a, 0x9b*/	CMD_MUTE, CMD_UP, CMD_ENTER, CMD_CHANNEL_INC,
/*0x9c, 0x9d ,0x9e, 0x9f*/	CMD_LEFT, CMD_DOWN, CMD_RIGHT, CMD_DISPLAY,
...
/*0xc0, 0xc1 ,0xc2, 0xc3*/	CMD_NOP, CMD_OPTION_RED, CMD_PLAY_PAUSE, CMD_STOP,
...
/*0xcc, 0xcd ,0xce, 0xcf*/	CMD_TELETEXT, CMD_NOP, CMD_SOURCE, CMD_PICTURE_MODE,
...
/*0xdc, 0xdd ,0xde, 0xdf*/	CMD_NOP, CMD_RETURN, CMD_NOP, CMD_NOP,
...
stTEACFAC_DTVKeymap		TVKeyMapTab_TEACFAC[]=
{
...
stTEACFAC_DTVKeymap		DTVKeyMapTab_TEACFAC[]=
{
//	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
//	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},
	{CMD_NOP,					CMD_NOP},
...
stTEACFAC_DTVKeymap		AVKeyMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
	{CMD_NOP,					CMD_NOP},
...
stTEACFAC_DTVKeymap		PCKeyMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
...
stTEACFAC_DTVKeymap		HDMIKeyMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
	{CMD_NOP,					CMD_NOP},
...
stTEACFAC_DTVKeymap		MEDIAKeyMapTab_TEACFAC[]=
{
	{CMD_GOTO,					CMD_OPTION_GREEN},
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_TT_TXCL,				CMD_NOP},
...
stTEACFAC_DTVKeymap		DVDKeyMapTab_TEACFAC[]=
{	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stTEACFAC_DTVKeymap		DefaultKeyMapTab_TEACFAC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stTEACFAC_DTVKeymap		TTKeyMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
...
stTEACFAC_DTVKeymap		RECKeyMapTab_TEACFAC[]=
{
...
stTEACFAC_DTVKeymap		ColorKeyReuseMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
	{CMD_NOP,					CMD_NOP},
...
stTEACFAC_DTVKeymap		ColorKeyATVReuseMapTab_TEACFAC[]=
{
	{CMD_PVR_FILE_LIST,			CMD_OPTION_GREEN},
	{CMD_PVR_SCHEDULE_LIST,		CMD_OPTION_YELLOW},	
	{CMD_NOP,					CMD_NOP},	
...
static stTEACFAC_DTVKeymap DVDKeyMapTab[]=
{
	{CMD_ENTER,                 0x1d},
	{CMD_DISPLAY,				0x01},	
	{CMD_STOP,					0x02},
...
/*************************************************************************************************************************************/
extern stTEACFAC_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stTEACFAC_DTVKeymap *pDVD_key = ( stTEACFAC_DTVKeymap *)DVDKeyMapTab;
...
int remote_TEACFAC_nec(int code)
{
	stTEACFAC_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	...
	extern bool TvApi_CurrentUnRSS (void);
#if defined(ENABLE_DVD_SOURCE)
	...
	if((code & 0x0000ffff) != TEACFAC_CUSTOMER)
	    return -1;
	...
	switch (source)
	{
			...
			//add by lq@2014.12.16--start
			{
				if(TvApi_GetIDtvSubSource() >= 1)
				{
					if( TvApi_IsRecording() == true )
						pKeymap=RECKeyMapTab_TEACFAC;
					else
						pKeymap=DTVKeyMapTab_TEACFAC;
					if( TvApi_CurrentUnRSS() == false && TvApi_IsRecording() != true)
						pKeymap = ColorKeyReuseMapTab_TEACFAC;
				}
				else
				{
					if( TvApi_IsRecording() == true )
						...
						pKeymap=TVKeyMapTab_TEACFAC;
					if( TvApi_CurrentUnRSS() == false && TvApi_IsRecording() != true)
						pKeymap = ColorKeyATVReuseMapTab_TEACFAC;
				}
			}
			//add by lq@2014.12.16--end
#else
		...
		case SOURCE_HDMI3:
			pKeymap=HDMIKeyMapTab_TEACFAC;
			break;
		
		case SOURCE_BROWSER:
		case SOURCE_OSD:
		case SOURCE_PLAYBACK://add by zjl@2014.06.11
			pKeymap=MEDIAKeyMapTab_TEACFAC;
			break;
...
#endif
		default:
			pKeymap=DefaultKeyMapTab_TEACFAC;
			break;
