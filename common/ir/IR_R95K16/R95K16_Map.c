...
#include "R95K16_Map.h"

typedef	struct _stR95K16_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stR95K16_DTVKeymap;
...
/*0x1c, 0x1d ,0x1e, 0x1f*/	CMD_SOURCE_VGA, CMD_NOP, CMD_NOP, CMD_SOURCE_HDMI1,
...
/*0x40, 0x41 ,0x42, 0x43*/	CMD_DISPLAY, CMD_SOUND_MODE, CMD_PICTURE_MODE, CMD_SLEEP,
/*0x44, 0x45 ,0x46, 0x47*/	CMD_DISPLAY_RATIO, CMD_NOP, CMD_NOP, CMD_PC_AUTO_ADJUST,
...
stR95K16_DTVKeymap		TVKeyMapTab_R95K16[]=
{
	{CMD_TELETEXT_SUBTITLE,		CMD_STITLE},
	{CMD_NOP,					CMD_NOP},
};

stR95K16_DTVKeymap		DTVKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
};

stR95K16_DTVKeymap		AVKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stR95K16_DTVKeymap		PCKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stR95K16_DTVKeymap		HDMIKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stR95K16_DTVKeymap		MEDIAKeyMapTab_R95K16[]=
{
	{CMD_MENU, 					CMD_QUICK},
	{CMD_EXIT,					CMD_RETURN},
	{CMD_TT_TXCL,				CMD_NOP},
...
stR95K16_DTVKeymap		DVDKeyMapTab_R95K16[]=
{	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stR95K16_DTVKeymap		DefaultKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stR95K16_DTVKeymap		TTKeyMapTab_R95K16[]=
{
...
stR95K16_DTVKeymap		RECKeyMapTab_R95K16[]=
{
	{CMD_NOP,					CMD_NOP},
};
...
static stR95K16_DTVKeymap DVDKeyMapTab[]=
{
...
/*************************************************************************************************************************************/
extern stR95K16_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stR95K16_DTVKeymap *pDVD_key = ( stR95K16_DTVKeymap *)DVDKeyMapTab;
...
int remote_R95K16_nec(int code)
{
	stR95K16_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	SourceOption source = SourceControl_GetCurSource();
	    
	if((code & 0x0000ffff) != R95K16_CUSTOMER)
	{    
		return -1;
	}
	
	printf("<Hikeen ir remote_R95K16_nec> customer_id=0x%x---key_code=0x%x\n",(code & 0x0000ffff),((code & 0x00ff0000)>>16));
	while(i <=0xFF)
	{
		if(i == ((code & 0x00ff0000) >> 16))
		{
			cDscIrKey=stR95K16_Keymap[i];
			break;
		}
		i++;
	}
	...
	switch (source)
	{
		...
		else	
		{
			if(TvApi_GetIDtvSubSource() >= 1)
			{
				...
				{	
					pKeymap=RECKeyMapTab_R95K16;
				}
				else
				{
					pKeymap=DTVKeyMapTab_R95K16;
				}
			}
			else
			{
		...
		case SOURCE_VGA1:
		pKeymap=PCKeyMapTab_R95K16;
		break;
		
		case SOURCE_HDMI1:
		case SOURCE_HDMI2:
		case SOURCE_HDMI3:
		pKeymap=HDMIKeyMapTab_R95K16;
		break;
		
		case SOURCE_BROWSER:
		case SOURCE_OSD:
		case SOURCE_PLAYBACK:
		pKeymap=MEDIAKeyMapTab_R95K16;
		break;
		...
		default:
		pKeymap=DefaultKeyMapTab_R95K16;
		break;	
	}
