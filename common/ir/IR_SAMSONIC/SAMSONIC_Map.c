...
#include "SAMSONIC_Map.h"

typedef	struct _stSAMSONIC_DTVKeyMAP
{
	int	oriIRKey;
	int	dstIRKey;
}stSAMSONIC_DTVKeymap;

int stSAMSONIC_Keymap[0x100] = {
/*0x00, 0x01 ,0x02, 0x03*/	CMD_ONE, CMD_TWO, CMD_THREE, CMD_POWER,
/*0x04, 0x05 ,0x06, 0x07*/	CMD_FOUR, CMD_FIVE, CMD_SIX, CMD_FAVCH,
/*0x08, 0x09 ,0x0a, 0x0b*/	CMD_SEVEN, CMD_EIGHT, CMD_NINE, CMD_NOP,
/*0x0c, 0x0d ,0x0e, 0x0f*/	CMD_NOP, CMD_ZERO, CMD_JUMP, CMD_DASH,

/*0x10, 0x11 ,0x12, 0x13*/	CMD_PICTURE_MODE, CMD_EPG, CMD_USB, CMD_PVR_RECORD,
/*0x14, 0x15 ,0x16, 0x17*/	CMD_CHANNEL_INC, CMD_NOP, CMD_VOL_UP, CMD_NOP,
/*0x18, 0x19 ,0x1a, 0x1b*/	CMD_CHANNEL_DEC, CMD_DISPLAY_RATIO, CMD_VOL_DOWN, CMD_NOP,
...
/*0x4c, 0x4d ,0x4e, 0x4f*/	CMD_OPTION_BLUE, CMD_PVR_TIMESHIFT, CMD_LIST, CMD_NOP,

/*0x50, 0x51 ,0x52, 0x53*/	CMD_REPEAT, CMD_TELETEXT_SUBTITLE, CMD_PLAY_PAUSE, CMD_STOP,
...
stSAMSONIC_DTVKeymap		TVKeyMapTab_SAMSONIC[]=
{
	{CMD_TELETEXT_SUBTITLE,		CMD_STITLE},
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--start
stSAMSONIC_DTVKeymap		DTVKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
};
//add by zjl@2014.09.01--end
stSAMSONIC_DTVKeymap		AVKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stSAMSONIC_DTVKeymap		PCKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stSAMSONIC_DTVKeymap		HDMIKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};
stSAMSONIC_DTVKeymap		MEDIAKeyMapTab_SAMSONIC[]=
{
	{CMD_EXIT,					CMD_RETURN},
...
stSAMSONIC_DTVKeymap		DVDKeyMapTab_SAMSONIC[]=
{	// IR_CMD				DVD_CODE
	{CMD_MUTE,					0x003C},
	{CMD_NOP,					CMD_NOP},
};
stSAMSONIC_DTVKeymap		DefaultKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
	{CMD_NOP,					CMD_NOP},
};	

stSAMSONIC_DTVKeymap		TTKeyMapTab_SAMSONIC[]=
{
	{CMD_PLAY_PAUSE,			CMD_TELETEXT},
	{CMD_STOP,					CMD_TT_HOLD},
	{CMD_REPEAT,			CMD_TT_INDEX},
	{CMD_FAVCH,				CMD_TT_REVEAL},
	{CMD_LIST,				CMD_WIDE},
	{CMD_NOP,					CMD_NOP},
};

//add by lq@2014.12.16--start
stSAMSONIC_DTVKeymap		RECKeyMapTab_SAMSONIC[]=
{
	{CMD_NOP,					CMD_NOP},
};
...
static stSAMSONIC_DTVKeymap DVDKeyMapTab[]=
{
...
/*************************************************************************************************************************************/
extern stSAMSONIC_DTVKeymap DVD_key[4];
extern unsigned int custome_code;
stSAMSONIC_DTVKeymap *pDVD_key = ( stSAMSONIC_DTVKeymap *)DVDKeyMapTab;
...
int remote_SAMSONIC_nec(int code)
{
	stSAMSONIC_DTVKeymap* pKeymap=NULL;
	int   cDscIrKey=0;
	int   cDscIrSWKey=0;
	int i =0;
	...
	if((code & 0x0000ffff) != SAMSONIC_CUSTOMER)
	    return -1;
	...
	switch (source)
	{
			...
			//add by lq@2014.12.16--start
			{
				if(TvApi_GetIDtvSubSource() >= 1)
				{
					if( TvApi_IsRecording() == true )
						pKeymap=RECKeyMapTab_SAMSONIC;
					else
						pKeymap=DTVKeyMapTab_SAMSONIC;
				}
				else
				{
					if( TvApi_IsRecording() == true )
		...
		case SOURCE_HDMI3:
			pKeymap=HDMIKeyMapTab_SAMSONIC;
			break;
		
		case SOURCE_BROWSER:
		case SOURCE_OSD:
		case SOURCE_PLAYBACK://add by zjl@2014.06.11
			pKeymap=MEDIAKeyMapTab_SAMSONIC;
			break;
...
#endif
		default:
			pKeymap=DefaultKeyMapTab_SAMSONIC;
			break;
